...
//   try {
//     const result = await supabase.query('SELECT * FROM public.users WHERE email = $1', [email]);
...
//     // Execute the query
//     const result = await supabase.query(query, [exercise_id, month, year, day]);
...
//     const result = await supabase.query(query, [user_id]);
...
//     // Execute the query
//     const result = await supabase.query(query, [month, year, day]);
//     res.status(200).json(result.rows);
//   } catch (err) {
//     console.error(err);
//     res.status(500).json({ error: 'Failed to retrieve exercises' });
...
//     const existingRecordResult = await supabase.query(existingRecordQuery, existingRecordParams);
...
//           await supabase.query(updateQuery, updateParams);
...
//         await supabase.query(resetQuery, resetParams);
...
//     const result = await supabase.query(query, updateParams);
...
//   try {
//     const result = await supabase.query('DELETE FROM public.exercise_log WHERE id = $1', [recordId]);
